...
export class UsuarioService {
    ...
    async findUsuarioById(idUsuario: string, manager?: EntityManager): Promise<Usuario | null> {
    ...
    async findUsuarioByEmail(email: string, manager?: EntityManager): Promise<Usuario | null> {
        const repo = this.getRepository(manager);
        return repo.findOne({
            where: { correo: email },
            relations: ['roles', 'cliente'],
    ...
    async registerUserCliente(usuarioDTO: CreateUsuarioDTO, idCliente: string): Promise<Usuario> {
      ...
      const existingUser = await this.usuarioRepository.findOne({ where: { correo: usuarioDTO.correo } });
      if (existingUser) {
      ...
      if (usuarioDTO.roles?.length) {
        ...
        const rolesIdsCliente = new Set(
          roles.map((rol) => rol.idRol),
        );
    ...
    async verifyUserCliente(idCliente: string, idUsuario: string): Promise<void> {
      ...
      try {
        // Buscar al usuario
        const usuarioVerificar = await this.findUsuarioById(idUsuario, queryRunner.manager)
        ...
        const cliente = await this.clienteService.findClienteById(idCliente, queryRunner.manager);
        if (!cliente) {
          throw new NotFoundException('Client not found');
        }

        // Verificar que el usuario pertenezca al cliente
        if (!usuarioVerificar.cliente || usuarioVerificar.cliente.idCliente !== idCliente) {
          throw new BadRequestException('The user does not belong to the specified client');
        }

        // Establecer estado de usuario como verificado
        usuarioVerificar.verificado = true;
        usuarioVerificar.estado = 'A';
